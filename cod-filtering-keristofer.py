print("Ａ Ｍ Ｉ Ｒ")
print(" **keristofer** ")
print("  pedar filtering  ")
import os
#What do you have in the source?
os.system("pip install colored")
os.system("pip install colorama")
os.system("pip install datetime")
#importing
import time
import sys
import colored
import colorama
import datetime
from colored import fg, bg, attr
from colorama import Fore, Back, Style
# colors
red='\033[31m'
green='\033[32m'
blue='\033[36m'
pink='\033[35m'
rang='\033[34m'
os.system("clear")
print(f"{red} ")
print(Fore.WHITE + "")
#date
x = str(datetime.datetime.now())
print("started " + (x))
print(f"{green} ")
print(" cod filtering ")
print(Fore.YELLOW + "")
x = """ Loading ~ ~ ~


n                     .             .                n.
  . .dP                   dP               9b               9b   .
 4  qXb         .        dX                 Xb       .      dXp   t
dX.  9Xb      .dXb     __                     __    dXb.   dXP   .Xb
9XXb._     _.dXXXXb dXXXXbo.               dXXXXb dXXXXb._   _.dXXP
 9XXXXXXXXXXXXXVXXXXXXXXOo.           .oOXXXXXXXXVXXXXXXXXXXXXXXXP
   9XXXXXXXXXXXXXXX ~   ~ OOO8b   d8OOO ~   ~ XXXXXXXXXXXXXXXXXP
     9XXXXXP   9XX      *     98v8P      *     XXP   9XXXXXXXP
      ~~~       9X.          .db|db.          .XP       ~~~
                  )b.  .dbo.dP' v  9b.odb.  .dX(
                ,dXXXXXXXXXXXb     dXXXXXXXXXXXb.
               dXXXXXXXXXXXP'   .    9XXXXXXXXXXXb
              dXXXXXXXXXXXXb   d|b   dXXXXXXXXXXXXb
              9XXb'    XXXXXb.dX|Xb.dXXXXX'    dXXP
                       9XXXXXX(   )XXXXXXP
                       XXXX X. v .X XXXX
                        XP^X' b   d' X^XX
                        X. 9         P )X
                         b          '  d'




"""
for c in x:
        sys.stdout.write(c)
        sys.stdout.flush()
        time.sleep(0.02)
print(f"{blue} ")
print("#.........")
time.sleep(0.9)
print("##........")
time.sleep(0.9)
print("###.......")
time.sleep(0.8)
print("####......")
time.sleep(0.7)
print("#####.....")
time.sleep(0.6)
print("######....")
time.sleep(0.5)
print("#######...")
time.sleep(0.4)
print("########..")
time.sleep(0.3)
print("#########.")
time.sleep(0.2)
print("##########")
time.sleep(0.1)
print(f"{pink} ")
print("Play - - - ! ")
print(Fore.YELLOW + "")
time.sleep(1.7)
print("  abar filtering (keristofer) ")
print(" ")
print("   chanel rubika = @aqa_keristofer  ")
print(" ")
print(f"{green} ")
print(" cod server ")
time.sleep(0.1)
print("server1= y//d/f")
time.sleep(1.0)
print("server2= g/d//")
time.sleep(0.1)
print(f"{red} ")
up = input("""

《 cod filter account 1 》

《 cod filter gap 2 》

《 cod filter chanel 3 》

 Which do you want >>> """)
print("  ")
print(f"{blue} ")
time.sleep(2.0)
if up == "1":
        print(' cod account = (4.6.4.0/i/h///s.6.3.2.5.7.9.9.7.6.4.2.1.4.6.5.3.2.5.7)')
if up != "1":
        print(' cod filter account ')
if up == "2":
        print(' cod gap = (5.2.7.9/g.p//d/h.6.4.2.5.6.9.4.2.4.6.3.2.4.6.7.5.3.2)')
if up != "2":
        print(' cod filter gap ')
if up == "3":
        print(' cod chanel = (4.6.9.1/g//h/i.4.3.2.5.6.7.9.7.4.1.4.5.7.4.2.2.5.7.4.2)')
if up != "3":
        print(' cod filter chanel ')
time.sleep(0.6)
print("""

         cod haro mahklot konid !

""")
print(f"{pink} ")
print(" cod ro 10 bar bezanid ke feik nashe ")
print("♡Thanks♡")
print(" ip rubika = 5.106.10.226 ")
print(f"{red} ")
print("""  love atrisa 💙  """)
print(" ")
print(" abar filtering rubika aqa amir ( keristofer ) ")
time.sleep(2.0)
print(""" Update Kardan script 👇:
    	
    	
rm -rf cod-filtering-keristofer 

git clone https://github.com/arthur0010/cod-filtering-keristofer 

cd cod-filtering-keristofer 

chmod 777 * 

python cod-filtering-keristofer.py

""")
print(" ")
print(" ostore filtering ")
print(" keristofer ")
print(" the end ")
time.sleep(2.0)
